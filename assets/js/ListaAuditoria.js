...
// ==========================================
// 1. CARGAR VISITAS PENDIENTES (TARJETAS)
// ==========================================
async function cargarVisitasPendientes() {
    ...
    if (res.status === 1 && res.data && res.data.length > 0) {
        ...
        // Mensaje si el Alcalde no tiene visitas
        contenedor.innerHTML = `
            <div class="col-12">
...
// ==========================================
// 2. CAMBIAR ESTADO (COMPLETAR / DENEGAR)
// ==========================================
async function cambiarEstadoVisita(id, nuevo_estado) {
    ...
    if(confirm(`¿Estás seguro de ${accion} esta solicitud?`)) {
        ...
        if (res.status === 1) {
            // Animación suave para hacer desaparecer la tarjeta
            ...
            if (tarjeta) {
                ...
                setTimeout(() => {
                    tarjeta.remove();
                    // Si ya no quedan tarjetas, mostramos el mensaje de éxito
                    const contenedor = document.getElementById('contenedor-visitas');
